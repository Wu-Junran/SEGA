...
class TUDataset_aug(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None,
                 ...
                 cleaned=False, aug=None, processed_filename='data.pt'):
        self.name = name
        ...
        '''
        bio_datasets = ['NCI1', 'NCI109', 'PROTEINS', 'DD', 'MUTAG', 'ENZYMES', 'PTC_MR']
        if self.name not in bio_datasets:
    ...
    def get(self, idx):
        ...
        for key in self.data.keys:
            if key == 'num_nodes':
                data.num_nodes = self.data._num_nodes[idx]
                continue
            item, slices = self.data[key], self.slices[key]
            if torch.is_tensor(item):
                s = list(repeat(slice(None), item.dim()))
        ...
        if data_aug.edge_index.shape[1] == 0:
            data_aug = drop_nodes(deepcopy(data))
            # data_aug = deepcopy(data)
...
if __name__ == '__main__':
    ...
    for d in social_datasets + bio_datasets:
        dataset = TUDataset_aug('data', name=d, aug='none')
        print(d, dataset)
        print(dataset[0])
