...
def svc_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if search:
            ...
            classifier = GridSearchCV(SVC(), params, cv=5, n_jobs=4, scoring='accuracy', verbose=0)
        else:
            classifier = SVC(C=10)
...
def randomforest_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if search:
            ...
            classifier = GridSearchCV(RandomForestClassifier(), params, cv=5, n_jobs=4, scoring='accuracy', verbose=0)
        else:
...
def linearsvc_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if search:
            params = {'C':[0.001, 0.01,0.1,1,10,100,1000]}
            classifier = GridSearchCV(LinearSVC(), params, cv=5, n_jobs=4, scoring='accuracy', verbose=0)
        else:
